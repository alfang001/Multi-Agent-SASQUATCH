...
from sqp import sqp_update, bfgs_update, agent_cost_func, finite_difference_gradient
from C_ADMM import agent_objective, phi, phi_derivative
...
def sqp_admm(measured_pos,adj_matrix, c_admm_max_iter = 200):
    ...
    for t in range(T):
        ...
        grad_f = [finite_difference_gradient(agent_cost_func, x_k[i]) for i in range(N)]
        x_new = []
        for i in range(N):
            x_var = cp.Variable(2)
